...
template<class ItemType>
void BST_312 <ItemType>::makeEmpty(TreeNode*& t)//need to fix this one
{
    if(t!=NULL) {
        if(t->left !=NULL) {
            makeEmpty(t->left);
...
int BST_312 <ItemType>::countNodes(TreeNode* t) const
{
    ...
    if(t!=NULL) {
        numberOfNodes++;
        if(t->left!=NULL) {
            numberOfNodes+=countNodes(t->left);
        }
        if(t->right!=NULL) {
            numberOfNodes+=countNodes(t->right);
        }
    }
...
{
    vector<ItemType> preOrderedVector;
    preOrderTraversal(root,preOrderedVector);
    return preOrderedVector;
...
{
    vector<ItemType> inOrderedVector;
    inOrderTraversal(root,inOrderedVector);
    return inOrderedVector;
}
...
{
    vector<ItemType> postOrderedVector;
    postOrderTraversal(root,postOrderedVector);
...
{
    bool flag=false;
    vector<ItemType> itemVector=this->preOrderTraversal();
